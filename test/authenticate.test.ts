...
describe("Authentication Middleware", () => {
    ...
    it("should call next with AuthenticationError when token is invalid", async () => {
        ...
        expect(nextFunction).toHaveBeenCalledWith(
            expect.objectContaining({
                message: "Unauthorized: Firebase Error: Invalid token",
                code: "UNKNOWN_ERROR",
            })
        );
        expect(mockResponse.locals).toEqual({});
    ...
    it("should handle AuthenticationError instances correctly", async () => {
        // Arrange
        mockRequest.headers = {
            authorization: "Bearer some-token",
        };

        const authError = new AuthenticationError("Custom auth error", "CUSTOM_AUTH_ERROR");
        (auth.verifyIdToken as jest.Mock).mockRejectedValueOnce(authError);
